...
def check_predictions_entity_aware_bert_fails_but_not_rbert(ground_truths, rbert_predictions, entity_bert_preds, test_sentences, save_path):
    failures = {}

    for i, (ground_truth, rbert_prediction, entity_bert_pred) in enumerate(zip(ground_truths, rbert_predictions, entity_bert_preds)):

        if rbert_prediction == ground_truth and entity_bert_pred != ground_truth:
            failures[i] = {
                'sentence' : test_sentences[i],
                'entity_bert_pred' : label2class[entity_bert_pred],
                'ground_truth' : label2class[ground_truth]
            }
            
    path_ = join_path(save_path, f'comparison_rbert_entity_bert_2.json')
    write_dict_to_json(failures, path_)
...
if __name__ == '__main__':
    ...
    entity_bert_preds = load_predictions(predictions_entity_att_bert)

    test_sentences = load_test_sentences(raw_test_data_path)

    check_predictions_rbert_fails_but_not_entity_aware_bert(ground_truths, rbert_predictions, entity_bert_preds, 
        test_sentences, evaluation_path)

    check_predictions_entity_aware_bert_fails_but_not_rbert(ground_truths, rbert_predictions, entity_bert_preds, 
        test_sentences, evaluation_path)