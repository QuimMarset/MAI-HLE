...
from models.entity_attention_bert import EntityAttentionBERT
from features_extractors.entity_attention_features_bert import EntityAttentionBERTFeatureExtractor
from utils.constants_paths import entity_attention_bert_config_path, words_vocabulary_path
...
class TrainerEntityAttentionBERT:
    ...
    def __init_config(self):
        self.config = read_yaml_config(entity_attention_bert_config_path)


    def __create_model(self, num_classes):
        num_positions = 2 * self.config.max_length + 1
        self.model = EntityAttentionBERT(num_classes, num_positions, self.config)
    ...
    def __create_feature_extractor(self):
        ...
        self.feature_extractor = EntityAttentionBERTFeatureExtractor(self.config.max_length, words_vocabulary)
